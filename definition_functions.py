...
class Solver:
    ...
    def calc_min_earliest_arrival_time(self, j, k):
        ...
        for i in arcs_to_j:
            if (i, j) in self.blacklist[j, k]:
                ...
                f_ij = self.earliest_feasible_arrival_times[i, j][-1]
    ...
    def do_recursion(self, i, j, earliest_feasible_at, time_gap=None):
        ...
        if len(previous_arcs) == 0 or all_blacklisted:
            ...
            # else, the backtracking needs to proceed further to see if the time at pr
                ...
                return self.check_for_feasibility(i, j, self.earliest_feasible_arrival_times[i, j][-1])
    ...
    def check_for_feasibility(self, i, j, earliest_feasible_at, add_park_time=None):
        ...
        if earliest_feasible_at == float('inf'):
            return float('inf')
        print("the time in class object is:", self.earliest_feasible_arrival_times[i,j])
        print("the time in function argument is:", earliest_feasible_at)
        ...
        if len(feasible_p_i) > 0:
            ...
            if add_park_time:
                ...
                if new_p_i <= max_p_i:
                    ...
                    # if the additional parking time is to big for the current node, the time at the succeeding node
                    # cannot be increased sufficiently through parking at the predecessor / current node.
                    # However, it can be checked whether it is possible to arrive at the current node at its former
                    #
                    if len(self.get_route_for_arc((i,j), self.opt_is)) > 1:
                        return self.do_recursion(i, j, earliest_feasible_at, self.parking[i, j][-1]+self.parking[i, j][-2])
                    elif len(self.get_route_for_arc((i,j), self.opt_is)) > 0:
                        return self.do_recursion(i, j, earliest_feasible_at, self.parking[i, j][-1])
                    else:
                        return float('inf')
